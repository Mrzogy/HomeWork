...
let explanation = """
The value of a variable is fixed and can never be modified or changed. The same value can be used elsewhere
The solution is to change the constant to a variable so that we can change the value whenever we want
"""
print(explanation)
